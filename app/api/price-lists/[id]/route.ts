...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) { const { id } = await params; await db.delete(priceLists).where(eq(priceLists.id, id)); return NextResponse.json({ ok: true }); }
