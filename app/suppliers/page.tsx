...
export default function Suppliers() {
  ...
  const load = () => fetch('/api/suppliers').then(r => r.json()).then(x => Array.isArray(x) && setRows(x));
  useEffect(() => { load(); }, []);
  ...
  const outstanding = ledger && selected ? +ledger.bills.find(b => b.id === selected)!.total - +ledger.bills.find(b => b.id === selected)!.paid : 0;
    ...
    <main className="management">
      <header className="management-head">
        <div>
          ...
          <h1>Suppliers</h1>
          <p>Manage trading partners, balances, and bill-wise payments.</p>
        </div>
      ...
      <section className="card data-panel">
        <ListFilters controls={controls} placeholder="Search supplier, mobile, or balance…" />
        <div className="supplier-table">
          ...
          {!controls.filtered.length && <p className="empty">No suppliers match these filters.</p>}
        </div>
        ...
        <div className="modal">
          ...
          <section className="supplier-form card">
            ...
            <form onSubmit={submit}>
              <label>Supplier name<input name="name" required defaultValue={form?.name} /></label>
              <label>Mobile number<input name="mobile" required defaultValue={form?.mobile} /></label>
              <label>Address<textarea name="address" defaultValue={form?.address} /></label>
              <label>Opening balance (AED)<input name="openingBalance" type="number" min="0" step=".01" required defaultValue={form?.openingBalance ?? '0'} /></label>
              {error && <div className="form-error">{error}</div>}
              <button className="primary">{form ? 'Update supplier' : 'Save supplier'}</button>
            </form>
          </section>
        </div>
      )}
      ...
      {view && (
        <div className="modal document-modal">
          ...
          <section className="document-modal-card card">
            <button className="sheet-close no-print" onClick={() => setView(null)}>
              <X />
            </button>
      ...
      {remove && (
        <div className="modal">
          ...
          <section className="supplier-form card">
            ...
            <p>This will permanently remove {remove.name} if it has no purchase history.</p>
            {error && <div className="form-error">{error}</div>}
            <div className="confirm-actions">
              <button className="outline" onClick={() => setRemove(null)}>Cancel</button>
              <button className="delete-button" onClick={del}>Delete</button>
            </div>
          </section>
        </div>
      )}
        ...
        <div className="modal ledger-modal">
          ...
          <section className="ledger card">
            ...
            <div className="ledger-totals">
              <span>Opening<b>{money(ledger.summary.openingBalance)}</b></span>
              <span>Purchases<b>{money(ledger.summary.totalPurchases)}</b></span>
              <span>Payments<b>{money(ledger.summary.totalPaid)}</b></span>
              <span>Payable<b>{money(ledger.summary.payable)}</b></span>
            </div>
            {ledger.bills.map(b => (
              <div className="ledger-line" key={b.id}>
                <span><b>{b.invoiceNumber}</b><small>{shortDate(b.purchaseDate)}</small></span>
            ...
            <form
              ...
              onSubmit={async e => {
                e.preventDefault();
                setError('');
                const d = new FormData(e.currentTarget), amount = Number(d.get('amount'));
                if (!Number.isFinite(amount) || amount <= 0) return setError('Payment amount must be greater than AED 0.00.');
                if (amount > outstanding) return setError(`Payment cannot exceed the outstanding balance of ${money(outstanding)}.`);
                const r = await fetch(`/api/suppliers/${ledger.supplier.id}/payments`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ purchaseId: d.get('purchaseId'), date: d.get('date'), amount, method: d.get('method'), notes: d.get('notes') })
                });
                if (r.ok) { setLedger(await get(ledger.supplier.id)); setSelected(''); load(); }
                else setError((await r.json()).error);
              }}
            >
              <select name="purchaseId" required value={selected} onChange={e => setSelected(e.target.value)}>
                <option value="">Select purchase bill</option>
                {ledger.bills.filter(b => +b.paid < +b.total).map(b => (
                  <option key={b.id} value={b.id}>{b.invoiceNumber} · outstanding {money(+b.total - +b.paid)}</option>
                ))}
              </select>
              {selected && <p className="payment-hint">Outstanding for selected bill: <b>{money(outstanding)}</b></p>}
              <input name="date" type="date" required />
              <input name="amount" type="number" min=".01" max={selected ? outstanding : undefined} step=".01" required placeholder="Amount" />
              <select name="method">
                <option>Cash</option>
                <option>Card</option>
