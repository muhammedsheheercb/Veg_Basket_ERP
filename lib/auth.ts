...
export async function signSession(session: Session) { if (!process.env.AUTH_SECRET) throw new Error('AUTH_SECRET must be configured.'); return new SignJWT(session).setProtectedHeader({ alg: 'HS256' }).setIssuedAt().setExpirationTime('8h').sign(secret); }
